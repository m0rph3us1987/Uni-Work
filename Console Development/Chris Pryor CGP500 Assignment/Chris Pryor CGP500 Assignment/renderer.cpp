...
#if (__ORBIS__)
size_t sceLibcHeapSize = 48 * 1024 * 1024; // 24[MB]
#endif
...
#pragma pack()
			...
			for (int i = 0; i<outBMPData->width*outBMPData->height; ++i)
			{
				...
				// Test - draw transparent elements as yellow
				//if ( (pix & 0xFF000000) == 0x00 )
				//{
				//	pix = 0xff00ffff;
				//}
	...
	static const Vector4			kClearColor = Vector4(0.0f, 0.0f, 0.0f, 1);
	...
	// Allow us to create a blank texture for fonts/off-screen rendering
	// or procedural patterns/images
	void Mesh::CreateBlankTexture()
	{
		// Allocate the texture data using the alignment 256
		void* pixels = s_garlicAllocator.allocate(kDisplayBufferWidth*kDisplayBufferHeight * 4, 256);
		DBG_ASSERT_MSG(pixels, "Cannot allocate file data %d\n", 0);

		memset(pixels, 0xff, kDisplayBufferWidth*kDisplayBufferHeight * 4);

		texture = new Gnm::Texture();
		texture->initAs2d(kDisplayBufferWidth,
	...
	void Mesh::BuildDrawBuffer()
	{
		// Save how many items we have to draw
		kIndexCount = iData.size();
		DBG_ASSERT(kIndexCount>0);

		// 1000 - is to allow for the dynamically changing vertex/index data
		// instead of re-allocating


		// Allocate the vertex buffer memory
		vertexData = static_cast<Vertex*>(s_garlicAllocator.allocate(
			vData.size() * sizeof(Vertex)* 10000,
		...
		indexData = static_cast<uint16_t*>(s_garlicAllocator.allocate(
			iData.size() * sizeof(uint16_t)* 10000,
			Gnm::kAlignmentOfBufferInBytes));
	...
	} // End BuildBuildBuffer(..)



	void Mesh::RebuildDrawBuffer()
	{
		...
		DBG_ASSERT(vData.size()<1000);
		DBG_ASSERT(iData.size()<1000);
		...
		vData.clear();
		iData.clear();
	}// End RebuildDrawBuffer(..)


	void Mesh::DrawLine(const Vector3& v0,
		const Vector3& v1,
		const Vector3& c/*=Vector3(1,1,1)*/)
	{
		AddVertex(Vertex(v0.getX(), v0.getY(), v0.getZ(), c.getX(), c.getY(), c.getZ(), 0.0f, 0.0f));
		AddVertex(Vertex(v1.getX(), v1.getY(), v1.getZ(), c.getX(), c.getY(), c.getZ(), 0.0f, 0.0f));

		AddIndex(this->GetVertexCount() - 2);
		AddIndex(this->GetVertexCount() - 1);
	...
	/*
	3 indices for a triangle list
	2 indices for a line list
	*/
	...
	{
		return kDisplayBufferWidth;
	}// End GetWidth(..)


	int Renderer::GetHeight() const
	{
		return kDisplayBufferHeight;
	}// End GetHeight(..)
	...
	void Renderer::Create()
	{
		...
		sampler.setWrapMode(Gnm::kWrapModeMirror, Gnm::kWrapModeMirror, Gnm::kWrapModeMirror);
	...
	void Renderer::RenderLoop()
	{
		...
		// Enable z-writes using a less comparison functionsc
		...
		primSetupReg.init();
		primSetupReg.setCullFace(Gnm::kPrimitiveSetupCullFaceNone); // Gnm::kPrimitiveSetupCullFaceBack);
		primSetupReg.setFrontFace(Gnm::kPrimitiveSetupFrontFaceCw); // ccw
		...
		for (int m = 0; m<meshes.size(); ++m)
		{
			...
			//DBG_ASSERT(scale.getY() > 0 && scale.getY()<10000.0f);
			//DBG_ASSERT(scale.getZ() > 0 && scale.getZ()<10000.0f);

			//DBG_ASSERT(rotation.getX() > -100000 && rotation.getX()< 100000);
			//DBG_ASSERT(rotation.getY() > -100000 && rotation.getY()< 100000);
			//DBG_ASSERT(rotation.getZ() > -100000 && rotation.getZ()< 100000);
			...
			if (constants)
			{
				...
				DBG_ASSERT(pos.getZ() == 0);
				Vector3 size = Vector3(scale.getX(), scale.getY(), scale.getZ());
				DBG_ASSERT(size.getZ() == 1);

				// We map the unit-cube render space to (0,0 640,480) screen space
				Matrix4 worldViewProj =
					//Matrix4::scale( Vector3(1,-1,1) ) // top-left of screen ccw
					//*
					Matrix4::translation(Vector3(-1, -1, 0))
					*
					Matrix4::scale(Vector3(1 / 640.0f, 1 / 480.0f, 1))
					*
					Matrix4::translation(pos * 2.0f)
					*
					Matrix4::scale(size)
					*
					Matrix4::translation(Vector3(1, 1, 0))
					*
					Matrix4::scale(Vector3(1, -1, 1));  // bottom-left of screen 
				;

				constants->m_WorldViewProj = ToMatrix4Unaligned(worldViewProj);

#if 0
				const Matrix4 scaleMatrix = Matrix4::scale(Vector3(1.0f / (kDisplayBufferWidth  * 0.5f),
					1.0f / (kDisplayBufferHeight * 0.5f),
					1));
				const Matrix4 transMatrix = Matrix4::translation(Vector3(-1.0, -1.0, 0));
				constants->m_WorldViewProj = ToMatrix4Unaligned(transMatrix * scaleMatrix);
#endif
			...
			gfxc.setPrimitiveType(meshes[m]->primitiveType); // lines or triangles
