...
namespace Solent
{
	...
	{
	public:
		// Storage/handler variables for rendering each object
		bool					enabled = false;
		Gnm::PrimitiveType		primitiveType = Gnm::kPrimitiveTypeTriList;
		...
		// Clearing or updating the vertex/index buffers for the GPU
		void ClearDrawList();
		void BuildDrawBuffer();
		void RebuildDrawBuffer();
	...
	public:
		...
		// Depending on the primative type - index the
		// vertices to create lines or triangles
		void AddIndex(const int i0);

		// Draw line calls AddVertex and AddIndex to create a line
		void DrawLine(const Vector3& v0,
			const Vector3& v1,
			const Vector3& c = Vector3(1, 1, 1));

		// Lets us know how many vertices have been added through AddVertex
		int GetVertexCount() const;

		// Lets us know how many indices have been added through AddIndex
